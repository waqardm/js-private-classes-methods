...
Classes can have constructor functions and methods:
...
If you've worked with other object-oriented languages, you'll agree that this is a great addition to the language. But as you start working more with Classes in JavaScript you will find yourself missing something that’s common in other languages: **private fields**. Until now JavaScript hasn't supported private fields on classes, but there’s a [Stage 3 TC39 proposal](https://tc39.github.io/proposal-class-fields/) to change this. Before we talk about these, a quick refresher on public fields.

## Public fields

We can rewrite our example from above to use a public field. By declaring a public field we can ensure the field is always present, and the class definition is more self-documenting.

You can choose to provide a default value or leave it undefined.
...
Let's take a look at the syntax of private fields. You may have seen code in the past that used `_` as a prefix to indicate that a property or method was private.

Well, when you want to prevent others from accessing a property outside of your class, **# is the new _**. To declare that we want a field in our class to be private, we use a # name (said "hash name"). Check out this example:
...
While there are obvious similarities in syntax between public and private fields, it's important to note the differences. Unlike public fields, private fields are _not_ properties. Instead, they make use of scoping to restrict access.

It's useful to remember that the `#` is part of the name itself, and it must be used when declaring and accessing the field.
...
We can successfully set `nonExistentPublicField` here because while there are good reasons to declare public fields up front, it's not strictly necessary. Private fields on the other hand make use of lexical scoping and so _must be declared_. Setting `#nonExistentPrivateField` fails above because it is a syntax error to refer to # names not in scope.
...
We publish our library and document that Pokémon info must be fetched using the `getPokemon` method. Unfortunately, despite this - we can't stop the user from getting Pokémon in other, unsupported ways. In fact there are four ways they could access them:
...
Now, with the code above, the only source that the user can get the Pokémon info is by using the only public method: `getPokemon`. This way, we can totally change how our library works under the hood, as long as the `getPokemon` method keeps its behavior.
